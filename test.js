...
{
	expect(window.isDOMVoidElement).to.be.a("function");
}));
...
it("returns false for a non-HTMLElement", runInBrowser(() =>
{
	...
	fixtures.forEach(fixture => expect(isDOMVoidElement(fixture)).to.be.false);
}));



it("returns false for a non-void HTMLElement", runInBrowser(() =>
{
	["a", "b", "div"].forEach(tagName =>
	{
		expect(isDOMVoidElement(document.createElement(tagName))).to.be.false;
		expect(isDOMVoidElement(anotherDocument.createElement(tagName))).to.be.false;
	});
}));



it("returns true for a void HTMLElement", runInBrowser(() =>
{
	["br", "hr", "input"].forEach(tagName =>
	{
		expect(isDOMVoidElement(document.createElement(tagName))).to.be.true;
		expect(isDOMVoidElement(anotherDocument.createElement(tagName))).to.be.true;
	});
}));
